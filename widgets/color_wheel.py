from PySide6.QtWidgets import QWidget, QPushButton, QColorDialog, QVBoxLayout
from PySide6.QtGui import QColor
from PySide6.QtCore import Qt, Signal

class ColorWheel(QWidget):
    color_change = Signal(QColor)

    def __init__(self):
        super().__init__()
        self.setMaximumHeight(30)
        self.current_color = QColor(Qt.GlobalColor.black)
        color_button = QPushButton("Choose Color")
        color_button.setFixedSize(150, 30)
        color_button.clicked.connect(self.choose_color)

        layout = QVBoxLayout(self)
        layout.setContentsMargins(1,1,1,1)
        layout.addWidget(color_button)
    
    def choose_color(self):
        color = QColorDialog.getColor(self.current_color, self, options = QColorDialog.DontUseNativeDialog and QColorDialog.ShowAlphaChannel)
        if color.isValid():
            self.current_color = color
            self.color_change.emit(color)